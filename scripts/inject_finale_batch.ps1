$baseDir = "c:\Development\Owner\CarMoneyPit\src\main\resources\data"
$modelsPath = Join-Path $baseDir "car_models.json"
$marketPath = Join-Path $baseDir "model_market.json"
$reliabilityPath = Join-Path $baseDir "model_reliability.json"
$faultsPath = Join-Path $baseDir "major_faults.json"
...
function Update-JsonFile {
    ...
    $current = Get-Content $path -Raw -Encoding UTF8 | ConvertFrom-Json
    $existingIds = $current.id
    if ($key -eq "model_id_ref") { $existingIds = $current.model_id_ref }
    elseif ($path -like "*market*" -or $path -like "*reliability*") { $existingIds = $current.model_id }
    
    $toAdd = @()
    foreach ($item in $data) {
        $id = $item.id
        if ($key -eq "model_id_ref") { $id = $item.model_id_ref }
        elseif ($path -like "*market*" -or $path -like "*reliability*") { $id = $item.model_id }
    ...
    if ($toAdd.Count -gt 0) {
        $current += $toAdd
        $current | ConvertTo-Json -Depth 10 | Set-Content $path -Encoding UTF8
        Write-Host "Added $($toAdd.Count) items to $path."
    } else {
        Write-Host "No new items for $path."
    }
}

Update-JsonFile $modelsPath ($finaleData | ForEach-Object { $_.model_def }) "id"
Update-JsonFile $marketPath ($finaleData | ForEach-Object { $_.market }) "model_id"
Update-JsonFile $reliabilityPath ($finaleData | ForEach-Object { $_.reliability }) "model_id"
Update-JsonFile $faultsPath ($finaleData | ForEach-Object { $_.faults }) "model_id_ref"
